...
void loop() {
  ...
  if (methanSerial.available()) {
    ...
    // uint8_t ack = methanSerial.read();
    // uint8_t responseLb = methanSerial.read();
    // uint8_t responseCmd = methanSerial.read();
    // uint8_t df1 = methanSerial.read();
    // uint8_t df2 = methanSerial.read();
    // uint8_t st1 = methanSerial.read();
    // uint8_t st2 = methanSerial.read();
    // uint8_t responseCs = methanSerial.read();

    // Serial.print(ack);
    // Serial.print(" ");
    // Serial.print(responseLb);
    // Serial.print(" ");
    // Serial.print(df1);
    // Serial.print(" ");
    // Serial.print(df2);
