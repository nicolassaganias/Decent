...
float calibrationOffset = 0.0;  // Add this variable for calibration
...
float readTemp() {
  ...
  if (addr[0] != 0x10 && addr[0] != 0x28) {    // check address recognition
    Serial.print("Device is not recognized");  // print recognition error
    return -1000;                              // return error
  }
