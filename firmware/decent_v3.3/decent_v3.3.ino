...
void loop() {
  ...
  // Verificar si han pasado 8 horas y la bomba no está encendida
  if (!liquidPump3On && (currentMillis - lastPumpStartTime >= PUMP_INTERVAL)) {
    lastPumpStartTime = currentMillis;
    pumpOnTime = currentMillis;  // Guardar el tiempo de encendido
    liquidPump3On = true;
    digitalWrite(LIQUID_PUMP3, HIGH);
    Serial.println("🟢 Liquid Pump 3 ENCENDIDA por 5 segundos");
  }

  // Verificar si la bomba lleva encendida 5 segundos para apagarla
  if (liquidPump3On && (currentMillis - pumpOnTime >= PUMP_ON_TIME)) {
    liquidPump3On = false;
    digitalWrite(LIQUID_PUMP3, LOW);
    Serial.println("🔴 Liquid Pump 3 APAGADA");
  }
  ...
  if (!manualMode) {  // automatic mode
    ...
    if (abs(pHSensor1Data - pHSensor2Data) <= PH_DIFFERENCE_THRESHOLD) {
      Serial.println("⚠ MEMBRANE BROKEN ⚠)");
    } else {}

    // Liquid Pump 2 behaviour. Se enciende la Pump solo si Ph es menor a 6,5 o myor a 8. También espera 10 minutos antes de volver a medir
    if (pHSensor1Data > 0 &&
    ...
    if (startCooldown && (currentMillis - cooldownStartTime >= 600000)) {  // 10 min en ms = 600000
      startCooldown = false;
...
void toggleLP3() {
  liquidPump3On = !liquidPump2On;  // Cambiar estado
  analogWrite(LIQUID_PUMP3, liquidPump3On ? 140 : 0);  // ON a 140, OFF a 0

  Serial.print("🔄 Liquid Pump 3 Alternando cada 8hs: ");
  Serial.println(liquidPump3On ? "ON" : "OFF");
}
